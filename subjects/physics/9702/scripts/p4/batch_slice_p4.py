#!/usr/bin/env python3
"""Batch Image and Mark Scheme Extractor for Cambridge Physics 9702 Paper 4 (A Level Structured Theory).

Extracts all question images, diagrams, and mark scheme slices for all 69 Paper 4 papers (2016 to 2025)
...
PHYSICS_P4_DIR = REPO_ROOT / "subjects/physics/9702/past papers/p4"
SCRIPTS_P4_DIR = Path(__file__).resolve().parent

if str(SCRIPTS_P4_DIR) not in sys.path:
    sys.path.insert(0, str(SCRIPTS_P4_DIR))

import slice_p4_paper

PAPER_RE = re.compile(
    r"^(?P<subject>\d{4})_(?P<session>[msw])(?P<year>\d{2})_qp_(?P<variant>4\d)$",
    re.I,
)


def get_all_p4_papers(min_year: int = 2016, max_year: int = 2025) -> list[dict]:
    """Find and return all complete P4 papers sorted chronologically."""
    papers = []
    for qp_pdf in sorted(PHYSICS_P4_DIR.rglob("*_qp_*.pdf")):
...
def run_batch(
    ...
    for idx, paper in enumerate(batch_papers, 1):
        ...
        t0 = time.time()
        try:
            slice_p4_paper.process_theory_paper(qp_pdf, ms_pdf, p_dir, dpi=dpi)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Batch Image and Mark Scheme Extractor for Physics 9702 Paper 4 (Theory)")
    ...
    all_papers = get_all_p4_papers(min_year=args.min_year, max_year=args.max_year)
    total_papers = len(all_papers)
    batch_size = args.batch_size
    batches = [all_papers[i : i + batch_size] for i in range(0, total_papers, batch_size)]
    total_batches = len(batches)

    print(f"Found {total_papers} total Paper 4 papers across {total_batches} batches (batch size: {batch_size}) for years {args.min_year}-{args.max_year}.")
